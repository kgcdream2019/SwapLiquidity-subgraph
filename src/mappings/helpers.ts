...
export function convertBnbToDecimal(bnb: BigInt): BigDecimal {
  return bnb.toBigDecimal().div(exponentToBigDecimal(BI_18))
}
...
export function createLiquiditySnapshot(position: LiquidityPosition, event: ethereum.Event): void {
  ...
  snapshot.token0PriceUSD = (token0.derivedBNB as BigDecimal).times(bundle.bnbPrice)
  snapshot.token1PriceUSD = (token1.derivedBNB as BigDecimal).times(bundle.bnbPrice)
